...
export class UsersService {
  ...
  public submitForIdentification({
    id,
      ...
      switch (identificationStatus) {
        case UserIdentificationStatus.ACCOUNT_IDENTIFIED:
          throw new UnauthorizedException(
            'Your account is already identified, Thanks!',
          );
        case UserIdentificationStatus.IDENTIFICATION_IN_PROGRESS:
          throw new UnauthorizedException(
            'Your account is under verification, Please wait!',
          );
        default:
          throw new UnauthorizedException(
  ...
  public async identifyUser({ email }: IdentifyUserDto) {
    ...
    if (
      ...
      switch (user.identificationStatus) {
        ...
        case UserIdentificationStatus.ACCOUNT_IDENTIFIED:
          throw new UnauthorizedException(
            `Account is already identified, What's wrong Admin?`,
          );
        ...
        default:
          throw new UnauthorizedException(
            'Please wait until the user upload his identication files',
          );
      }
    ...
    return this.update(
      { id: user.id },
      {
        identificationStatus: UserIdentificationStatus.ACCOUNT_IDENTIFIED,
      },
    );
  }
}
