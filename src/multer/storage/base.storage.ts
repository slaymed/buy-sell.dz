...
export abstract class BaseStorage {
  ...
  public async validateFileContent(
    ...
    return (
      typeResult &&
      this.validateMimeType(typeResult.mime) &&
      this.validateExtension(typeResult.ext)
    );
  }

  public async validate(
    file: Express.Multer.File,
  ): Promise<{ validFile: boolean; error: string | null }> {
    const allowedFileSize = this.validateFileSize(file);
    if (!allowedFileSize)
      return { validFile: false, error: 'Size not allowed' };

    const validFileContent = await this.validateFileContent(file);
    if (!validFileContent)
      return { validFile: false, error: 'Content does not match extension' };
